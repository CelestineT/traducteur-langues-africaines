...
def afficher_audio_local(chemin_audio):
    """Affiche un fichier audio local"""
    if not chemin_audio:
        return False
    ...
    try:
        # Méthode 1 : st.audio direct (le plus simple)
        st.audio(chemin_audio, format='audio/mp3')
        return True
    except Exception as e:
        # Méthode 2 : Lire et encoder en base64
        try:
            with open(chemin_audio, 'rb') as audio_file:
                ...
                audio_html = f'''
                <audio controls style="width:100%" playsinline webkit-playsinline>
            ...
            st.error(f"❌ Impossible de charger l'audio : {e2}")
            return False
