...
def afficher_audio_local(chemin_audio, audio_id="default"):
    ...
    try:
        ...
        # ID unique avec timestamp pour éviter le cache
        import time
        timestamp = int(time.time() * 1000)
        unique_id = f"audio_{audio_id}_{timestamp}"
        
        # Player HTML optimisé pour Safari iOS et tous les mobiles
        audio_html = f'''
        <div style="margin: 10px 0;" id="container_{unique_id}">
            <audio 
                id="{unique_id}"
                controls 
            ...
            >
                <source src="data:audio/mpeg;base64,{audio_base64}?t={timestamp}" type="audio/mpeg">
                <source src="data:audio/mp3;base64,{audio_base64}?t={timestamp}" type="audio/mp3">
